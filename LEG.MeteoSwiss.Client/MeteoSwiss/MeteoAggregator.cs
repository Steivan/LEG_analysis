        ...
        public static void RunMeteoAggregationForPeriod(string stationId, StationMetaInfo stationMetaInfo, int periodStartYear, int periodEndYear, string granularity = "t", bool isTower = false)
        {
            ...
            var monthlyAverages = filteredRecords
                ...
                .Select(g => new
                {
                    ...
                    GlobalRadiation = SafeAverage(g.Select(r => r.GlobalIrradiance)),
                    DiffuseRadiation = SafeAverage(g.Select(r => r.DiffuseIrradiance)),
            ...
            var overallAverage = new
            {
                ...
                GlobalRadiation = SafeAverage(filteredRecords.Select(r => r.GlobalIrradiance)),
                DiffuseRadiation = SafeAverage(filteredRecords.Select(r => r.DiffuseIrradiance)),
