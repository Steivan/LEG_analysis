...
namespace LEG.PV.Data.Processor
{
    public class DataImporter
    {
            ...
            ImportE3DcAndMeteoHistory(int folder, bool meteoTillNow = false) 
        {
            ...
            var (timeStamps, directGeometryFactors, diffuseGeometryFactor, cosSunElevations, installedPower) = await pvProduction(siteId, firstTimestamp, lastTimestamp, minutesPerPeriod, shiftSupportTimeStamps: 0);
        ...
        public async Task<(
           ...
           double installedPower,
           int periodsPerHour)>
            ImportMeteoForecastAndCalculatedProduction(int folder, DateTime firstE3DcTimestamp, DateTime lastHistoryTimestamp)
        {
            folder = 1 + (folder - 1) % 2;
            var siteId = folder == 1 ? ListSites.Senn : ListSites.SennV;
            ...
            var lastTimestamp = firstTimestamp.AddDays(10);
            ...
            ImportE3DcHistoryAndCalculated(int folder, int displayPeriod = 2)      // 0: downloaded E3Dc history, 1: meteo history till now, 2: including meteo forecast
        {
            ...
            if (displayPeriod == 2)
            {
                ...
                var lastHistoryTimestamp = dataRecordsHistory[^1].Timestamp;
                var (perStationWeatherForecast, blendedWeatherForecasty, _, dataRecordsForecast, validRecordsForecast, _, _) = await ImportMeteoForecastAndCalculatedProduction(folder, firstE3DcTimestamp, lastHistoryTimestamp);
            ...
            foreach (var stationId in selectedStationsIdList)
            {
                ...
                foreach (var record in blendedForecast)
                {
                    ...
                    weatherRecords.Add(new WeatherCsvRecord
                    {
                        ...
                        GlobalIrradiance = record.DniWm2,
