...
{
    public class PlotProductionProfiles
    {
        internal static void ProductionProfilePlot(SolarProductionAggregateResults productionResults)
        {
            // Prepare data
                ...
                for (var roof = 1; roof <= productionResults.DimensionRoofs; roof++)
                {
                    ...
                    for (var hour = 0; hour < 24; hour++)
                    {
                        ...
                        theoreticalProductionPerMonth[hour] += theoreticalRelativeProductionPerMonthAndRoof[hour] *
                                                         productionResults.PeakPowerPerRoof[roof - 1]; // kW
                        effectiveProductionPerMonth[hour] +=
                            effectiveRelativeProductionPerMonthAndRoof[hour] * productionResults.PeakPowerPerRoof[roof - 1]; // kW
                    }
            ...
            var hourSupport = Enumerable.Range(0, 23).Select(h => (h + 0.5)).ToArray();
            for (var month = 1; month <= 12; month++)
            {
